...
[FIGURE PLACEHOLDER: "AQUA-OS platform stack overview showing hardware → separation kernel → APEX services → I/O mgr/HM → partitions. Include callouts for DAL levels and DO-297 roles. Style: clean technical infographic, blueprint palette."]
...
14. [Glossary](#glossary--ata-42-os-aqua-os)
...
[FIGURE PLACEHOLDER: "Context diagram: aircraft systems ↔ AQUA-OS partitions (FCC, NAV, DISPLAYS, MAINT) over AFDX/ARINC 429. Highlight separation boundaries. Style: blueprint network diagram."]
...
[FIGURE PLACEHOLDER: "Layered architecture diagram with components: SK, PM, APEX, I/O Manager, HM, Config Loader; arrows for control/data paths. Include trust boundary. Style: UML-like, blueprint palette."]
...
[FIGURE PLACEHOLDER: "Poster-style tile grid of design principles with small icons and one-line callouts. Style: monochrome blueprint, print-friendly."]
...
[FIGURE PLACEHOLDER: "Major/minor frame timeline (Gantt-like) with labeled windows (FCC_A, NAV, DISPLAYS, MAINT). Show jitter band and constraints. Style: clean, blueprint."]
...
[FIGURE PLACEHOLDER: "Address space schematic: kernel region, partition A/B/C regions, shared memory with R/W policy labels and deny arrows. Style: blueprint memory map."]
...
[FIGURE PLACEHOLDER: "HM concept view: detectors → classifier → action selector (LOG/HOT/WARM/STOP/SYS RECONFIG). Include telemetry to audit log. Style: block diagram."]
...
[FIGURE PLACEHOLDER: "Side-by-side explainer: Sampling vs Queuing. Sampling shows last-value overwrite + validity timer; Queuing shows FIFO depth, overflow policy. Style: blueprint infographic."]
...
[FIGURE PLACEHOLDER: "I/O virtualization: physical NIC → virtual NICs per partition; AFDX policing (BAG/Lmax). Include ARINC 429 label map inset. Style: blueprint network."]
...
[FIGURE PLACEHOLDER: "Boot flow/state machine: COLD → INIT → SCHEDULED; with branches to WARM/HOT and MAINTENANCE. Add guard conditions (hash OK, config valid). Style: state diagram, blueprint."]
...
[FIGURE PLACEHOLDER: "Annotated schedule example: offsets, durations, window boundaries, IPC deadlines within windows. Include legend. Style: timeline blueprint."]
...
[FIGURE PLACEHOLDER: "Sequence diagram for overrun handling: Process overruns → Scheduler detects → HM action (HOT restart) → Resume schedule. Style: UML sequence, blueprint."]
...
[FIGURE PLACEHOLDER: "Timing chart: producer updates sampling port; consumer reads with validity window; show stale detection. Style: timing waveform."]

**Queuing Ports**

* Bounded depth; overflow policy: drop/flag with HM event.

[FIGURE PLACEHOLDER: "Queuing behavior: enqueue/dequeue over time with depth counter, overflow event to HM. Style: timeline + inset FIFO."]
...
[FIGURE PLACEHOLDER: "Time sync topology: grandmaster → switch fabric → CPIOM → partitions (read-only). Include drift alarm path to HM. Style: network/time diagram."]
...
[FIGURE PLACEHOLDER: "AFDX mapping view: partitions → VNics → VL IDs with BAG/Lmax; show policing and traffic shaping. Style: blueprint rack/network."]

[FIGURE PLACEHOLDER: "ARINC 429 bus map: multiple receivers/transmitters with label/SDI routing table. Style: simple bus schematic."]
...
[FIGURE PLACEHOLDER: "HM escalation ladder: INFO → WARNING → MAJOR → FATAL mapped to actions (LOG/HOT/WARM/STOP/SYS RECONFIG). Include detection latency budget. Style: ladder infographic, blueprint."]
...
[FIGURE PLACEHOLDER: "Config integrity chain: schema → authoring → signing → bootloader hash check → runtime lock. Show artifacts: schedule.xml, memory_map.xml, HM_Table.xml, io_map.yaml. Style: flow with padlock icons (minimal), blueprint."]
...
[FIGURE PLACEHOLDER: "Compliance matrix heatmap: rows (ARINC 653, DO-178C, DO-330, DO-297, ARP4754B/4761A, DO-326A/356A/355); columns (Scope, Evidence, Status). Style: minimalist heatmap, blueprint."]
...
[FIGURE PLACEHOLDER: "Traceability chain: Requirements → Design → Tests → Results → Coverage → Compliance. Show evidence artifacts (logs, reports) with hashes. Style: V-model ribbon, blueprint."]
...
[FIGURE PLACEHOLDER: "Compact acronym wall: ARINC/DO/ARP families grouped with tiny definitions (no full text), purely decorative divider for glossary. Style: subtle, blueprint micro-typography."]
